...
public class LFragmentCouponAdd extends BaseFragment implements View.OnClickListener {
    private ClearEditText etCouponCode;
    private ImageButton ibSearch;
    private TextView tvType, tvFaceValue, tvMinMoney, tvIsWithVip, tvIsWithSpecialDish;
    private TextView tvClose, tvConfirm;
    private TextView tvLabel1;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View mView = inflater.inflate(R.layout.fragment_coupon_add, container, false);
        initView(mView);
        setListener();
        initData(null);
        return mView;
    ...
    private void setViewClickable(boolean clickable) {
        tvConfirm.setClickable(clickable);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tv_coupon_confirm:
                EventBus.getDefault().post(new CouponAddConfirmEvent((OffLineWxCouponModel) tvConfirm.getTag()));
                break;
            case R.id.ib_search:
