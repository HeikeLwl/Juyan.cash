...
public class PageIndicatorView extends LinearLayout {
    ...
    public void initIndicator(int count) {
        ...
        if (indicatorViews.size() > 0) {
            indicatorViews.get(0).setBackgroundResource(R.drawable.presence_online);
        }
    }
    ...
    public void setSelectedPage(int selected) {
        for (int i = 0; i < indicatorViews.size(); i++) {
            if (i == selected) {
                indicatorViews.get(i).setBackgroundResource(R.drawable.presence_online);
            } else {
                indicatorViews.get(i).setBackgroundResource(R.drawable.presence_invisible);
            }
        }
    }
