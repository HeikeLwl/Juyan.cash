...
import jufeng.juyancash.adapter.SnackOrderMessageAdapter;

/**
 * Created by 15157_000 on 2016/6/24 0024.
 */
public class CustomeSnackOrderMessageDialog {
    private Context mContext;
    private AlertDialog mAlertDialog;
    private RecyclerView mRecyclerView;
    private TextView tvConfirm,tvClearAll;
    private SnackOrderMessageAdapter mAdapter;
    private OnOrderMessageClearListener mOnOrderMessageClearListener;

    public CustomeSnackOrderMessageDialog(Context context){
        this.mContext = context;
        mAlertDialog = new AlertDialog.Builder(mContext).create();
        mAlertDialog.show();
        Window window = mAlertDialog.getWindow();
        window.setContentView(R.layout.dialog_layout_snack_order_message);
        initView(window);
        setAdapter();
        setListner();
    }
    ...
    private void setAdapter(){
        mAdapter = new SnackOrderMessageAdapter(mContext.getApplicationContext());
